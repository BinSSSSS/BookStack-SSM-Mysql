						...
						<form action="${pageContext.request.contextPath}/user/login"
							...
							<p>
								<label for="ipt-account" class="uname" data-icon="u">
									...
									required="required" type="text" placeholder="账号、手机号码、用户名"
						...
						<form action="${pageContext.request.contextPath}/user/register"
							...
							<p>
								...
								<input id="usernamesignup" name="userName" required="required"
									...
									for="usernamesignup" class="error-tips"> <c:if
										test="${not empty user_tips}">${user_tips}</c:if>
								</label>
							</p>
							<p>
							...
							<p>
								<label for="passwordsignup_confirm" class="youpasswd"
									...
									id="passwordsignup_confirm" name="password" required="required"
									type="password" placeholder="eg. X8df!90EO" /> <label
									for="passwordsignup_confirm" class="error-tips"> </label>
							</p>
							<p>
								<label class="youcode">验证码：</label> <input name="checkCode"
									type="text" style="width: 120px; padding-left: 5px;">
								<%-- <img
									src="${pageContext.request.contextPath}/user/checkCode" alt="验证码"
									class="check-code"> --%>
								<button type="button" class="check-code sender disable"
									id="msg-sender" disabled="disabled">发送验证码</button>
								<label for="passwordsignup_confirm" class="error-tips"
									style="display: block; margin: 5px 0 0 0"> <c:if
										test="${not empty code_tips}">${code_tips}</c:if>
								</label>
							</p>
